...
/**
 * Class that represents the Game Model. All towers, mobs, projectiles and
 * information about the current level is stored here.
 * 
...
public class GameModel {
	...
	public static void initialize(Context context) {
		...
		for (int i = 0; i < sPath.getSize() - 1; i++) {
			...
			while (Coordinate.getDistance(temp, c2) > 2) {
				...
				sOccupiedTilePositions.add(new Point((int) temp.getX()
						/ GAME_TILE_SIZE, (int) temp.getY() / GAME_TILE_SIZE));

				sOccupiedTilePositions.add(new Point((int) temp.getX()
						/ GAME_TILE_SIZE + 1, (int) temp.getY()
						/ GAME_TILE_SIZE));
				...
				temp.setXY(temp.getX() + Math.cos(angle),
						temp.getY() - Math.sin(angle));
	...
	/**
	 * Adds a tower to a specific place on the game field. The added tower is
	 * placed on a grid, consisting of tiles with the size
	 * GAME_TILE_SIZExGAME_TILE_SIZE
	 * 
	 * @param x
	 *            Tile position on the X-axis
	 * @param y
	 *            Tile position on the Y-axis
	 */
	public static void buildTower(Tower tower, int x, int y) {

		tower.setCoordinates(new Coordinate(x * GAME_TILE_SIZE, y
				* GAME_TILE_SIZE));
	...
	/**
	 * Returns whether a tower can be placed on the gamefield with it's current
	 * coordinates
	 */
	 ...
	 * 
	 * @param setFast
	 *            True sets the game in fast forward mode, false sets the game
	 *            to normal speed.
	 */
	...
	/**
	 * Toggles fast forward by changing the game's speed multiplier to the
	...
	/**
	 * This class is called each frame. It keeps track of the creation of the
	 * mobs from the waves of the current map. Called from updateModel.
	 * 
	 * @param timeDelta
	 */
	...
	/**
	 * This class is called from the GameThread. It updates the state of all
	 * towers, mobs and projectiles. It also handles projectile collisions with
	 * mobs dying and such.
	 * 
	 * @param timeDelta
	 */
	public static void updateModel(float timeDelta) {
		...
		if (GAME_STATE == STATE_RUNNING) { // Only update if running
			...
			if (mMobFactory.lastWaveHasEntered() && GameModel.sMobs.isEmpty()
					&& mCreatedLastWave && mMobFactory.getLastMobSent()) {
				mSelectedTower = null;
				mCurrentSnowball = null;
				mCurrentTower = null;
				mShowTooltip = false;
			...
			/*
			 * for every tower: create a new Projectile set to a Mob that the
			 * Tower can reach and add that to the list of Projectiles in the
			 * GameModel
			 * 
			 * tryToShoot() returns null if the tower can't reach any mob or if
			 * the tower is on CD
			 */
			...
			for (int i = 0; i < size - removed; ++i) {
				...
				// this is fulkod to keep projectiles that never reached their
				// target from staying on the gamefield
			...
			/*
			 * For every snowball: update position do damage to any mob it hits
			 */
			for (int j = 0; j < GameModel.sSnowballs.size(); ++j) {
				...
				for (int k = 0; k < deadMobs.size(); k++) {
					...
					case Mob.HEALTHY:
						deadMob.setHealth((int) (0.993 * deadMob.getHealth()));
						break;
					default:
						deadMob.setHealth((int) (0.93 * deadMob.getHealth()));
						break;
			...
			/*
			 * For every mob: Update position If the mob has died, handle it
			 */
